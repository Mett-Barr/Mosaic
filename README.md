...
> **目前狀態：資料層的垂直切片。** 領域模型、把 Spaceflight News 的回應變成文章的
> 容錯 mapper、以及讀一頁文章的網路來源都已就位並有測試。
> **畫面上還看不到任何東西**——沒有 repository、沒有 DI 接線、沒有 UI，
> 所以 `installDebug` 裝起來仍然是空的 app。
...
| 被丟掉的資料列要報給誰 | 原因已經帶回呼叫端，但還沒有人接 | UI 有錯誤呈現的位置之後 |

...
> **目前狀態：文章的完整流程已接通，還沒有異質內容與 freshness。**
> 清單、分頁、內頁、存起來、以及**存起來的文章沒有網路也打得開**。
> loading／empty／error／offline 四種狀態各有自己的畫面。
> **尚未在實機驗證過**：所有證據來自單元測試與建置，畫面本身還沒有自動化測試（見下方延後表）。
> 還沒有的：異質內容（第二種來源）、freshness policy、下拉更新。
...
**還沒做的** —— 下拉更新（讀者主動要求時應該無視政策）、
以及 `updated_at`（伺服器說文章何時被改過，可以讓「新鮮」更精確而不必重抓整頁）。

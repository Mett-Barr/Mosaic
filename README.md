...
> **第三種來源（TMDB 熱門電影）在模擬器上配著真資料看過了**——海報、評分、片名都是真的。
> 沒有 token 的 build 不會顯示那一列，這是刻意的（`DECISIONS.md` 40）。
> 還沒有的：搜尋與篩選。
...
**打開一篇文章不是第四條規則。** feed 交出去的那一頁留在記憶體裡，
讀者點開其中一張卡片時就從那裡回答，不再問一次網路——因為那張卡片**就是那篇文章畫出來的**。
它沒有自己的時間窗：**清空它的事件，就是上面那條規則裡「有人要第一頁」的那個事件**，
所以它記得的永遠是螢幕上那一代清單，不可能比螢幕舊。程序被殺掉之後它是空的，
那一次照樣重抓（`DECISIONS.md` 41）。
...
人  審測試 ← 支點
AI  commit 測試（紅）→ 實作到綠
    ./gradlew build detekt lint 全綠
AI  第二個模型獨立審 diff，任務是提出反對意見
人  審 diff ← 支點
AI  commit 實作（綠）
```

**人做五件事**：定義「對」、審測試、審 diff、架構取捨——以及**在裝置上看**。

最後一件不是形式。這個專案的畫面沒有自動化測試（`DECISIONS.md` 20 記錄了為什麼），
所以轉場與版面的唯一驗收方式是人打開 app。**實際發生的缺陷全部是這樣找到的**：
容器展開分兩段、標題出現疊影、返回時縮圖瞬間變圓、整個清單的卡片圓角一起在動。
**測試一個都沒抓到，兩個獨立審查的模型也都沒有**——它們讀得到程式碼，看不到畫面。

其中一次還推翻了我自己建立的模型。我以為標題被三層淡入相乘，據此做的「修正」
移除了唯一存在的那一層（配對成功的共享元素會被抬進 overlay，脫離父層的 alpha）。
**是實跑的畫面推翻它的**，而修正的依據是放慢十倍錄影、逐幀比對像素。

用**兩個不同實驗室**的模型互審，是因為同一個模型的第二意見會複製它自己的盲區。
它們有一次對同一份 diff 得出相反結論，而那個矛盾用物理解決了——細節在
[`AI_USAGE.md`](AI_USAGE.md)。

[`AGENTS.md`](AGENTS.md) 描述完整的開發迴圈，以及人與 AI 的分工落在哪裡。

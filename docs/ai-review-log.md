...
## 測試的等待方式：Codex 抓到的是我自己剛剛在找的那種東西，第三個

這一輪的 diff 是兩件事：`TmdbTrendingTest` 的兩個 `Thread.sleep` 輪詢迴圈換成
把 `MockEngineConfig.dispatcher` 設成測試自己的 dispatcher；以及兩個「斷言不到東西」
的斷言改寫。**生產程式碼一行都沒動**，送審的是測試與 `DECISIONS.md` 65。

**派工方式**：任務書寫在 repo 外的暫存目錄、`"$(cat ...)"` 餵、`< /dev/null`、
`--model gpt-5.6-luna -c model_reasoning_effort="max" --dangerously-bypass-approvals-and-sandbox`、
`-o` 落在 repo 外。任務書列了六條**指名要它去攻擊的主張**（等價性、急切執行的副作用、
`HttpTimeout` 會不會變成混淆因子、還有沒有不會失敗的斷言、某個斷言是不是噪音、
65 則有沒有把話講滿）。耗時約 40 分鐘。

### 它說了什麼、我怎麼做

| 它說的 | 我怎麼做 |
|---|---|
| **`assertEquals("and pays for it straight away", 0L, currentTime)` 不會失敗**：`runTest` 從虛擬時間零開始，`runCurrent()` 不推進時鐘，所以這一句對任何實作都是真的 | **原封不動接受，而且這是這一輪最有價值的一條。** 我整輪都在找「不會失敗的斷言」，找了兩個，走過第三個沒看見。刪掉它，把「立刻」併進上一句的訊息裡——`runCurrent()` 不推進時鐘，所以「跑完之後計數是一」本身就是「立刻」。用一個 mutation 證明併過去之後還抓得到：在第一次嘗試前面插一個 `delay`，紅在 `the first reader pays for one attempt, and pays now expected:<1> but was:<0>`；而被刪掉的那一句在同一個 mutation 下**仍然會綠** |
| **`4.744 / 300 = 15.8 ms` 是一整圈的價錢，不能算在 `Thread.sleep(1)` 頭上**：一圈還含 `runCurrent`、channel 輪詢與排程 | **接受。** 65 則改成「一圈約 15.8 毫秒」，另起一段說明睡眠大概是其中最大的一項但沒有分開量過，並註明結論不靠這個歸因 |
| 控制組與 mutation 的結果是「報告出來的測量」，不是留在 repo 裡可重跑的東西 | **接受。** 65 則的限制多一條，寫明重做的方法在對應 commit message 裡（改哪一行、預期紅在哪一句） |
| **`HttpTimeout` 這一輪不是問題，但只是因為 handler 都當場回答**：15 秒的 request timeout 現在也走虛擬時鐘，長跨度都發生在 `TmdbTrending` 已經在 `delay` 的時候 | **接受這個限定。** 我自己也推過同樣的結論，但沒有寫下來。65 則補一條限制：這是這個檔案**現在**的性質，不是它保證的性質 |
| `runCurrent()` 只看得到落在 test scheduler 上的工作，所以理論上仍可能有 false green——但這不是 Ktor 3.5.1 的現況 | **無需改動**，65 則的限制第一條已經是這句話。它確認了措辭沒有講滿 |
| 急切執行讓這個檔案不再觀察得到排程順序；至於 `while (true)` 會不會遞迴，它逐條核對三條路徑都通到正的 `delay`，判定是臆測 | **無需改動**，限制第二條已經寫了。**它的路徑核對是額外收穫**——那件事我沒有逐條查過 |
| `assertNotNull(films.lastProblem)` **是承重的，不是噪音**：`requests++` 發生在 `respondError` 之前，所以只有這一句能確認拒絕真的穿過 Ktor 走到 `fetch` 的失敗路徑 | **接受它推翻我的問法。** 我在任務書裡把它列成「是不是噪音」，它說不是並給了理由。維持現狀 |

**這一輪最值得記的**：任務書問「還有沒有不會失敗的斷言」，而整輪工作的主題**就是**
在找不會失敗的斷言。它找到第三個，在我改過的那一段裡，離我刪掉的那兩個各隔幾行。
上一輪的教訓是「讀了原始碼還是會把推論寫成測量」；這一輪是**正在找某一類缺陷，
不會讓你對那一類缺陷免疫**——尤其當它藏在你剛剛親手改過的五行之內。
